...
- `task-explorer`: deeper read-only tracing role for execution flow, contracts, invariants, and cross-file causality
...
### Parent prompt for `task-scout`

```text
Spawn one `task-scout` child for TASK_ID <TASK_ID>.

Scope:
- <one bounded lookup, mapping, or proof question>
...
- paths inspected
- direct findings
- unresolved questions that may require a deeper explorer or builder follow-up
```

### Parent prompt for `task-explorer`

```text
Spawn one `task-explorer` child for TASK_ID <TASK_ID>.

Scope:
- <one bounded execution-path, contract, or causality question>

Rules:
- Stay read-only
- Trace the real code path and relevant invariants
- Cite concrete files, symbols, and transitions
- Do not write proof-loop artifacts

Return only:
- paths and symbols inspected
- real execution or data flow
- constraints, invariants, and risks relevant to the next build or verify step
```

### Parent prompt for `task-worker-lite`

```text
Spawn one `task-worker-lite` child for TASK_ID <TASK_ID>.

Scope:
- <one narrow implementation or check shard>
...
### Parent prompt for `task-worker-strong`

```text
Spawn one `task-worker-strong` child for TASK_ID <TASK_ID>.

Scope:
- <one bounded but non-trivial implementation shard>
...
Rules:
- Edit only the allowed paths
- Handle bounded multi-file or ambiguity-prone work
- Keep unrelated files untouched
- Do not write proof-loop artifacts
- Do not spawn child agents
