...
- `task-scout` for ownership, file/symbol/test lookup, path mapping, and narrow proof probes
- `task-explorer` for execution tracing, contract mapping, invariant discovery, and cross-file causality
...
Purpose:
- Cheapest read-only lookup role for “where is it” questions

### `task-explorer`

Purpose:
- Deeper read-only tracing role for “why does it behave this way” questions

### `task-worker-lite`

Purpose:
- Narrow low-risk implementation helper with explicit ownership

### `task-worker-strong`

Purpose:
- Stronger bounded implementation helper for multi-file or ambiguity-prone work
...
- If implementation splits cleanly, add bounded `task-worker-lite` or `task-worker-strong` children in parallel with explicit file or module ownership.
- If proof needs multiple read-only probes, add bounded `task-scout` or `task-explorer` children in parallel, but keep verdict writing with one fresh verifier child.
