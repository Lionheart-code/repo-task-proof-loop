...
- In Codex, helper routing should be deliberate and cost-aware. Apply the minimum-sufficient model policy: keep `task-router` and `task-spec-freezer` on `gpt-5.4-mini` for bounded subagent work, use `task-scout` for cheap read-only lookup work, `task-explorer` for deeper read-only tracing, `task-worker-lite` for narrow low-risk edits, and keep stronger code-facing roles such as `task-worker-strong` and `task-verifier` on `gpt-5.4`. Keep inherited parent strength on the integration owner.
